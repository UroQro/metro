...
export default function PatientDetail({ patient: initialP, onClose }) {
  ...
  useEffect(() => { if(!editingNoteId) setNote({}); }, [noteType]);

  const saveNote = async () => {
     if(Object.keys(note).length === 0) return alert("La nota está vacía");
     if (editingNoteId) {
         const updatedNotes = p.notes.map(n => n.id === editingNoteId ? { ...n, content: note, type: noteType } : n);
         await updateDoc(doc(db, "patients", p.id), { notes: updatedNotes });
         setEditingNoteId(null); alert("Nota Actualizada");
     } else {
         const newNote = { id: Date.now(), type: noteType, author: auth.currentUser.displayName, date: new Date().toISOString(), content: note };
    ...
    <div className="bg-slate-100 min-h-screen pb-20">
       <div className="bg-white p-3 sticky top-0 z-30 shadow-sm flex items-center justify-between">
           ...
           <div className="flex gap-2">
               ...
               <button onClick={()=>setShowEdit(true)} className="bg-blue-50 text-blue-500 p-2 rounded-lg border border-blue-100 flex items-center gap-1"><Edit size={16}/> EDITAR</button>
           </div>
       </div>
       ...
       <div className="p-3 space-y-4">
           ...
           <div id="note-creator" className={`bg-white p-4 rounded-xl shadow-lg border-2 transition ${editingNoteId ? 'border-orange-400 ring-2 ring-orange-100' : 'border-blue-100'}`}>
               <div className="flex justify-between items-center mb-3">
                   <h3 className={`text-xs font-black uppercase ${editingNoteId ? 'text-orange-500' : 'text-blue-500'}`}>{editingNoteId ? 'EDITANDO NOTA...' : 'NUEVA NOTA'}</h3>
               ...
               <div className="space-y-2 mb-3">
                   {noteType === 'visita' && <>
                       ...
                       <div className="grid grid-cols-3 gap-2">
                           <NoteInput label="TA" k="ta" form={note} setForm={setNote}/> <NoteInput label="FC" k="fc" form={note} setForm={setNote}/> <NoteInput label="Temp" k="temp" form={note} setForm={setNote}/> 
                       </div>
                       <div className="grid grid-cols-2 gap-2">
                           <NoteInput label="Gasto U (ml)" k="gu" form={note} setForm={setNote}/> <NoteInput label="Drenaje (ml/tipo)" k="dren" form={note} setForm={setNote}/>
                       </div>
                       <div className="bg-slate-50 p-2 rounded border">
                           <p className="text-[9px] font-black text-slate-400 mb-1">LABORATORIOS COMPLETOS</p>
                           <div className="grid grid-cols-5 gap-1.5 mb-1.5">
                               <NoteInput label="Hb" k="hb" form={note} setForm={setNote}/> <NoteInput label="Htc" k="htc" form={note} setForm={setNote}/> <NoteInput label="Leu" k="leu" form={note} setForm={setNote}/>
                               <NoteInput label="Plt" k="plt" form={note} setForm={setNote}/> <NoteInput label="Glu" k="glu" form={note} setForm={setNote}/>
                           </div>
                           <div className="grid grid-cols-5 gap-1.5">
                               <NoteInput label="BUN" k="bun" form={note} setForm={setNote}/> <NoteInput label="Cr" k="cr" form={note} setForm={setNote}/>
                   ...
                   {noteType === 'check_qx' && <div className="space-y-1">
                       {['No Derechohabiencia','VPO','Labs','Cultivos','TAC','Pruebas Cruzadas','Hojas Consentimiento'].map(item=>(<label key={item} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={note[item]||false} onChange={e=>setNote({...note, [item]:e.target.checked})}/> {item}</label>))}
                   </div>}
                   
                   {noteType === 'check_egreso' && <div className="space-y-1">
                       {['Receta','Hojas de Egreso','Nota de Alta','Cita'].map(item=>(<label key={item} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={note[item]||false} onChange={e=>setNote({...note, [item]:e.target.checked})}/> {item}</label>))}
                   </div>}

                   {noteType === 'labs' && <div className="grid grid-cols-4 gap-2">{['Hb','Htc','Leu','Plt','Glu','Urea','BUN','Cr','Na','K','Cl','TP','TTP','INR'].map(k=><NoteInput key={k} label={k} k={k} form={note} setForm={setNote}/>)}</div>}
                   ...
                   {(noteType === 'texto' || noteType === 'proc' || noteType === 'img' || noteType === 'vpo') && <textarea className="w-full p-2 border rounded text-xs" rows="3" placeholder="Detalles..." value={note.text||''} onChange={e=>setNote({...note, text:e.target.value})}/>}
               </div>
               <button onClick={saveNote} className={`w-full text-white p-3 rounded-lg font-black uppercase text-xs flex items-center justify-center gap-2 ${editingNoteId ? 'bg-orange-500' : 'bg-slate-900'}`}><Save size={16}/> {editingNoteId ? 'ACTUALIZAR NOTA' : 'GUARDAR NOTA'}</button>
           </div>
               ...
               {p.notes?.slice().reverse().map(n => (
                   <div key={n.id} className={`bg-white p-3 rounded-xl shadow-sm border ${editingNoteId === n.id ? 'border-orange-400 bg-orange-50' : 'border-slate-200'}`}>
                       ...
                       <div className="text-sm text-slate-700">
                           ...
                           {n.type === 'inter' && <div><p className="font-black text-blue-800 uppercase mb-1">IC: {n.content.service}</p><p>{n.content.text}</p></div>}
                           {(n.type === 'sonda' || n.type === 'abx') && <div><p className="font-bold text-lg">{n.content.med || 'Sonda/Catéter'}</p><p>Inicio: {n.content.date} <span className="bg-slate-900 text-white px-2 rounded-full text-xs">Día {calculateDays(n.content.date)}</span></p></div>}
                           ...
                           {n.type.includes('check') && <div className="grid grid-cols-2 gap-1">{Object.entries(n.content).map(([k,v])=>(v && <span key={k} className="flex items-center gap-1 text-xs"><CheckSquare size={12}/> {k}</span>))}</div>}
